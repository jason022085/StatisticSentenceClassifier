# -*- coding: utf-8 -*-
"""
Created on Sat Jun  8 17:52:18 2019
...
import numpy as np
#%%
DT = np.load('D:\Anaconda3\mycode\MLlab\DT_tfidf_4000.npz')#取前200字的DT_ns(有加入統計字典的)
X_train = DT["article_train"]
X_test = DT["article_test"]
y_train = DT["board_train"]
y_test = DT["board_test"]
#%%
X_train = X_train.reshape((40000,1,4000))
X_test = X_test.reshape((1416,1,4000))
...
def buildRNN():
    ...
    model = Sequential()
    model.add(CuDNNGRU(units= 128,return_sequences=True,input_shape=(1,4000),kernel_initializer="glorot_normal"))#首層的unit>下一層的unit
...
def trainModel(model,X_train,y_train,X_test,y_test):
    ...
    reduce_learning_rate_by_acc = ReduceLROnPlateau(monitor='acc', factor=0.1, 
                                                    ...
                                                    min_lr=0)
    check_point_by_acc = ModelCheckpoint("D:\Anaconda3\mycode\MLlab\model_tfidf_rnn\model_tfidf_rnn_r1_{epoch:02d}_{acc:.3f}_{val_acc:.3f}.h5", 
...
for i in range(1,11):
    ...
    #存下訓練紀錄
    np.savez('history_tf_rnn_v'+str(i)+'_40000', val_loss=history_40000.history['val_loss'], 
...
for i in range(1,11):
    ...
    np.savez('history_tf_rnn_v'+str(i)+'_20000', val_loss=history_20000.history['val_loss'],
...
for i in range(1,11):
    ...
    np.savez('history_tf_rnn_v'+str(i)+'_10000', val_loss=history_10000.history['val_loss'], 
...
#%%這裡會隨時改
from keras.models import load_model
model= load_model(r"D:\Anaconda3\mycode\MLlab\model_selected\tf_nn\model_tf_nn_v2_40000_76_0.930_0.902.h5")
import numpy as np
history = np.load(r"D:\Anaconda3\mycode\MLlab\model_selected\tf_nn\history_tf_nn_v2_40000.npz")
